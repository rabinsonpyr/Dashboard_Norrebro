...
with st.expander("🕐 Add staff working hours", expanded=False):
    ...
    if not wh_preview.empty:
        wh_preview_display = wh_preview.copy()
        wh_preview_display["Date"] = pd.to_datetime(wh_preview_display["Date"]).dt.strftime("%Y-%m-%d")
        st.dataframe(
            wh_preview_display.sort_values("Date", ascending=False).head(10),
